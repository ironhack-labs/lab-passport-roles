...
/* GET edit */
router.get("/edit/:id", [ensureLoggedIn("/"), isTA("/")], (req, res, next) => {
...
/* POST edit */
router.post("/edit/:id", [ensureLoggedIn("/"), isTA("/")], (req, res, next) => {
  let id = req.params.id;

  const { name, description } = req.body;
  const updates = { name, description };
  Course.findByIdAndUpdate(id, updates).then(() => {
    res.redirect("/course");
  });
});
...
router.get("/view/:id", [ensureLoggedIn("/"), isTA("/")], (req, res, next) => {
  let id = req.params.id;
