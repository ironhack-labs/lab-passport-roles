...
exports.createCourseForm = async (req, res, next) => {
    ...
    res.render('auth/createCourse', {course})
}

exports.createCourse = async (req, res, next) =>{
  const {name, teacher, duration, classroom} = req.body
  await Course.create({name, teacher, duration, classroom})
  res.redirect('staffprofile')
}
...
exports.editCourse = async (req, res, next) =>{
    const {name, teacher, duration, classroom} = req.body 
    await Course.findByIdAndUpdate({name, teacher, duration, classroom})
    res.redirect('/staffprofile')
}
